...
import { History, Calendar, Clock, ChevronRight, BarChart2, FileText, ChevronLeft } from "lucide-react"; 
...
const UserAllExamHistoryPage = () => {
    ...
    const fetchHistory = async (page = 1) => {
        try {
            setLoading(true);
            const res = await get(`/exam-result/history/all?page=${page}&limit=10`);
             ...
             <div className="max-w-6xl mx-auto">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
                     <div>
                         ...
                         <p className="text-gray-600">Theo dõi toàn bộ quá trình luyện tập của bạn</p>
                     </div>
                     <div className="bg-white px-5 py-3 rounded-xl shadow-sm border border-gray-100 flex items-center gap-3">
                         ...
                         <div>
                             <p className="text-xs text-gray-500 uppercase font-semibold">Tổng lượt thi</p>
                             <p className="text-xl font-bold text-gray-900">{pagination.total} lần</p>
                         </div>
                     </div>
                </div>
                                            ...
                                            <tr key={item._id} className="hover:bg-gray-50 transition-colors group">
                                                ...
                                                <td className="p-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="p-2 rounded-lg bg-blue-50 text-blue-600 group-hover:bg-white group-hover:shadow-sm transition">
                                                            <FileText className="w-5 h-5" />
                                                        </div>
                                                        <div>
                                                            <p className="font-bold text-gray-900 line-clamp-1 max-w-[200px] md:max-w-xs">{exam.title?.text || exam.title || "Đề thi không xác định"}</p>
                                                            <p className="text-xs text-gray-500 bg-gray-100 inline-block px-1.5 py-0.5 rounded mt-1">{exam.type}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="p-4">
                                                ...
                                                <td className="p-4 text-center hidden md:table-cell">
                                                    <div className="text-sm text-gray-600 font-medium">
                                                        {item.numberOfCorrectAnswers}/{exam.numberOfQuestions} câu
                                                    </div>
                                                </td>
                                                <td className="p-4 text-center hidden md:table-cell">
                                                ...
                                                <td className="p-4 text-right">
                                                    <button 
                                                        onClick={() => navigate(`/exam/result/${item.examId?._id || item.examId}?resultId=${item._id}`)}
