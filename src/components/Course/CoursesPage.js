...
export default function CoursesPage() {
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        setLoading(true);
        const response = await getCategories("", "", "COURSE");
        if (response && response?.data) {
  ...
  const handleSortPriceAsc = () => updateURL("price", "asc");
  const handleSortPriceDesc = () => updateURL("price", "desc");
          ...
          >
            ...
            <ul className="space-y-1">
              {categories.map((category, index) => (
                <li key={index}>
                  <Link
                    to={`/courses?query=${category?.type}`}
