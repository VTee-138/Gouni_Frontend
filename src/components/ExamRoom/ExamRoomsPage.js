...
import ExamRoomCard from "./ExamRoomCard";
import { DoorOpen, Menu, X, Search } from "lucide-react";
...
export default function ExamRoomsPage() {
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        setLoading(true);
        const response = await getExamRooms(page, 6, query, sort, order, true);
  ...
  useEffect(() => {
    const handleFetch = async () => {
      try {
        setLoading(true);
        const response = await getCategories("", "", "EXAM_ROOM");
        if (response && response?.data) {
  ...
  const updateURL = (newSort, newOrder) => {
    ...
    navigate({ pathname: "/exam-rooms", search: params.toString() });
  };

  // Sort handlers
  const handleSortNewest = () => updateURL("createdAt", "desc");
  const handleSortOldest = () => updateURL("createdAt", "asc");
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="text-sm text-gray-500 mb-4">
          ...
          / <span className="text-gray-800 font-medium">Phòng luyện thi</span>
        </div>
          ...
          >
            ...
            <ul className="space-y-1">
              {categories.map((category, index) => (
                <li key={index}>
                  <Link
                    to={`/exam-rooms?query=${category?.type}`}
                  ...
                  >
                    <div className="flex items-start gap-2 w-full">
                      <DoorOpen size={16} className="flex-shrink-0 mt-[2px]" />
            ...
            <div className="bg-white rounded-2xl shadow-lg p-4 sm:p-6 mb-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h2 className="text-lg sm:text-xl font-bold text-gray-800">
                  {category === "EXAM_ROOM"
                    ? "Tất cả phòng luyện thi"
                    : "Tất cả phòng luyện nền tảng"}
                </h2>
              ...
              <div className="mt-3">
                <div className="relative max-w-md">
                  <input
                    type="text"
                    placeholder="Tìm kiếm phòng luyện thi..."
                    value={searchValue}
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 mb-8">
                  {examRoomsData.map((examRoom) => (
                    ...
                    >
                      <ExamRoomCard {...examRoom} category={category} />
                    </div>
                  ))}
                </div>
                ...
                {examRoomsData?.length > 0 ? (
                  ...
                  <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                    <div className="text-gray-400 mb-4">
                      <DoorOpen size={64} className="mx-auto" />
                    </div>
                    <Typography
                      fontSize={"20px"}
                      fontWeight={"500"}
                    ...
                    >
                      Không có phòng luyện thi nào được tìm thấy
                    </Typography>
                    <Typography
                      fontSize={"14px"}
                      color="text.secondary"
