...
const ExamFixedInfoPanel = ({
  ...
  // Kiểm tra xem câu hỏi hiện tại có phải MQ không
  const currentQuestionMQ = questionsMQ.find((e) =>
    e?.range.includes(currentMQ == 0 ? currentMQ + 1 : currentMQ)
  );

  // Nếu là câu hỏi MQ thì không hiển thị panel
  if (currentQuestionMQ) {
    return null;
  }

  return (
    <div className="hidden lg:block w-full sm:w-[432px] bg-white shadow-lg border-l border-gray-200 overflow-y-auto">
...
export default ExamFixedInfoPanel;
